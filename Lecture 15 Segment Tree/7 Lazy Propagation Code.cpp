...
void updateSegmentTreeLazy(int *tree, int *lazy, int low, int high, int startR, int endR, int currPos, int inc){
	...
	if(lazy[currPos] != 0){
		...
		// if currentNode is not a leaf node, we update the child nodes
		if(low != high){
...
int main(){
	...
	// incrementing 3 in the range arr[0, 3]
	updateSegmentTreeLazy(tree, lazy, 0, 3, 0, 3, 1, 3);
	cout << "Segment Tree after incrementing 3" <<endl;
	for(int i=1; i<12; i++)
		cout << tree[i] <<endl;
	
	cout << "Lazy Tree after incrementing 3" <<endl;
	for(int i=1; i<12; i++)
		cout << lazy[i] <<endl;
	
	// incrementing 2 in the range arr[0, 1]
	updateSegmentTreeLazy(tree, lazy, 0, 3, 0, 1, 1, 2);
	cout << "Segment Tree after incrementing 2" <<endl;
	for(int i=1; i<12; i++)
		cout << tree[i] <<endl;
	
	cout << "Lazy Tree after incrementing 2" <<endl;
	for(int i=1; i<12; i++)
		cout << lazy[i] <<endl;
